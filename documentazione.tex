...
\begin{tikzpicture}[shorten >=1pt,node distance=3cm,node font=\small,on grid,auto,initial text={begin}]
	...
	\path[->]
	(START) edge [bend left] node [above] {i start = '1'} (WWN)
	(WWN) edge (FWN)
	(FWN) edge [bend left] node {i data = 0} (DONE)
	(FWN) edge [bend left] node {i data $\ne$ 0} (CR)
	(CR) edge [bend left] (WR)
	(WR) edge (FR)
	(FR) edge [bend left](E12)
	(E12) edge [loop below] node [align=center] {w word index has\\any other value} ()
	(E12) edge [bend right] node [below,align=center] {\\\\w word\\index = 0 or 4} (WRITE)
	(WRITE) edge [bend right] node [below,align=center] {\\not\\finished \\reading} (E12)
	(WRITE) edge node {words left = 0} (DONE)
	(WRITE) edge [bend right] node [align=center] {finished\\reading} (CR)
	(DONE) edge (AD)
	(AD) edge node {i start = '0'} (START);
\end{tikzpicture}
